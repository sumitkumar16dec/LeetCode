...
class Solution {
public:
    TreeNode *func(vector<int> &preorder, int preStart, int preEnd, vector<int> &inorder, int inStart, int inEnd, unordered_map<int,int> &mapIn){
    ...
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        ...
        TreeNode *root= func(preorder, 0, preorder.size()-1, inorder, 0, inorder.size()-1, mapIn);
        return root;
    }
};
// TC: O(n), SC: O(n)
