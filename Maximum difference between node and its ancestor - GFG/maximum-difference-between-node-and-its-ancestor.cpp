 ...
 */
int maxDiffUtil(Node* t, int &mini){
    if(t==NULL) return INT_MAX;
    if(t->left==NULL && t->right==NULL) return t->data;
    int val = min(maxDiffUtil(t->left, mini), maxDiffUtil(t->right, mini));
    mini = max(mini, t->data-val);
...
{
    int mini = INT_MIN;
    maxDiffUtil(root, mini);
    return mini;
}
// https://www.youtube.com/watch?v=d3ceg_7NcEY