...
class Solution
{
    ...
    Node *copyList(Node *head)
    {
        ...
        // 1st STEP: make copy of each node and link them together side-by-side in a single list.
        while(iter!=NULL){
            front=iter->next;
        ...
        // 2nd STEP: assign random(arb) pointers for the copy nodes.
        iter=head;
        while(iter!=NULL){
            if(iter->arb!=NULL) iter->next->arb=iter->arb->next;
            iter=iter->next->next;
        }
